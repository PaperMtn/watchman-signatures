...
class TestSigs(unittest.TestCase):
    ...
    def test_location_format(self):
        print('Testing location options')
        signatures = load_signatures_slack()

        for sig in signatures:
            if 'slack' in sig.watchman_apps:
                assert isinstance(sig.locations, list), f'Signature {sig.name} has' \
                                                        f' incorrectly formatted Slack Watchman scopes'

    def test_location_content(self):
        print('Testing locations content')
        signatures = load_signatures_slack()

        for sig in signatures:
            if 'slack' in sig.watchman_apps:
                self.assertTrue(assert_not_empty(sig.locations), f'Signature {sig.name} has no scopes.'
                                                                 f' There must be at least one to be used with Slack Watchman')
